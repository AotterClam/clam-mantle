...
[![CI](https://github.com/AotterClam/clam-mantle/actions/workflows/ci.yml/badge.svg?branch=develop)](https://github.com/AotterClam/clam-mantle/actions/workflows/ci.yml)
...
>
> clam-mantle is co-developed with AI coding agents (Claude Code) and is
...
Agent-native headless CMS where AI agents are first-class authors — locked-grammar manifests, structured JSON diagnostics, and static `.md` mirrors every agent can crawl without auth. Most CMSes treat AI as a content editor; clam-mantle treats it as the developer.
...
- **Apps (OLTP)** — `clam-mantle` (this repo) and future apps. Build a content-driven web service by declaring atoms in YAML; the runtime ships dispatcher + auth + render + MCP for free.
...
> Note: the two halves intentionally live in different GitHub orgs (`AotterClam` for clam-mantle, `aotter` for aotter-clam) — different release cadences, different licensing posture.
...
You're an agent helping a (likely non-technical) user install or extend a clam-mantle project.
...
| Package | Role |
|---|---|
| `@aotterclam/clam-mantle-spec` | Spec engine — types + parse + validate + diagnostics + JSON-Schema → zod converter + CLI. Zero env deps. |
| `@aotterclam/clam-mantle-runtime` | Runtime engine — dispatcher + entry-writer + view executor + content-ops + render + MCP. Defines required adapter ports plus optional feature ports. |
| `@aotterclam/clam-mantle-admin-ui` | Admin SPA — React 19 + Vite + Tailwind v4. In development; ships in v0.1.0. |
| `@aotterclam/clam-mantle-cloudflare` | Cloudflare Workers adapter. Implements ports against D1 / KV / ASSETS. Ships `createAuth()` — the Better Auth-backed *default* implementation of the SDK's `Auth` contract (see [ADR-0014](docs/adr/0014-auth-better-auth-and-multi-tenant-mcp.md) § "Auth as contract, Better Auth as default"); replace by passing your own `Auth` instance. |
| `@aotterclam/clam-mantle-netlify` | **Stub.** Coming v0.2. Engineering forcing function: keeps `clam-mantle-runtime` adapter-agnostic. |
...
End-user starters live in the [`AotterClam/clam-mantle-starters`](https://github.com/AotterClam/clam-mantle-starters) monorepo. Real-user installs run `npx @aotterclam/create-clam-mantle <archetype>`, which downloads a pinned source tarball, merges `_common/` + `<archetype>/` into the user's directory, and initializes a fresh user-owned Git repo without `origin` set. Premium / per-customer starters live in the private sibling [`AotterClam/clam-mantle-starters-premium`](https://github.com/AotterClam/clam-mantle-starters-premium).

| Starter | Family | Status | What |
|---|---|---|---|
| [`clam-mantle-starters/publication`](https://github.com/AotterClam/clam-mantle-starters/tree/main/publication) | publication | v0.1.0 (available) | Owner-published content — landing pages, articles, docs-lite, project updates, basic contact form. Multi-locale posts/pages, Cloudflare Turnstile, per-slug `.md` mirror, llms.txt, SEO/AEO. |
| [`clam-mantle-starters/blank`](https://github.com/AotterClam/clam-mantle-starters/tree/main/blank) | — | v0.1.0 (available) | Headless API + MCP only. Drop-in backend for consumers bringing their own frontend (Next.js / Astro / native / partner). |
