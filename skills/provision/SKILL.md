...
The previous `mantle-poc` blog Skill successfully validated this flow. Preserve these details:
...
Only run the `blank` production proof after its `src/mantleConfig.ts` uses `WorkersOAuthVerifier`, `createOAuthProvider`, `OAUTH_KV`, `GITHUB_CLIENT_ID`, `GITHUB_CLIENT_SECRET`, and `ADMIN_GITHUB_LOGIN` like `starters/blog`.
