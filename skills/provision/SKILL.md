...
The values land in the project via `create-clam-cms` and the install skill. Confirm only what affects resource names or the OAuth identity:
...
3. Writes resource IDs, `PUBLIC_ORIGIN`, and the Turnstile site key into `wrangler.toml`; updates `src/clamConfig.ts` `origin`.
