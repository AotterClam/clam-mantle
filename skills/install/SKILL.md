...
You (the agent) are bootstrapping a new clam-cms consumer for the user. Read this entire file before touching the filesystem. clam-cms ships two starters in <https://github.com/AotterClam/clam-cms>; both deploy to Cloudflare Workers + D1 + KV.
...
- If the user mentions Astro specifically: `starters/astro` is on the v0.1.x roadmap (issue [#2](https://github.com/AotterClam/clam-cms/issues/2)). For now use `starters/blank` and have Astro consume `/api/views/*`.
...
cp -R .clam-tmp/starters/<blog-or-blank>/. .
rm -rf .clam-tmp

# Or, when published as a template:
# pnpm dlx @aotterclam/create-blog <name>
# pnpm dlx @aotterclam/create-blank <name>
```
