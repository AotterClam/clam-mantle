...
> If you're an AI working in a project that consumes `@aotterclam/clam-mantle-*`,
  ...
  schema:
    ...
    properties:
      ...
      locale:   { type: string }   # NOT in `required:`; the runtime locale
                                   # gate (clam-mantle-runtime helpers.ts, per
...
`@aotterclam/clam-mantle-spec` as the `CLAM_REF_KEYWORD` constant; SDK
...
**Implementation status**: declared in
`@aotterclam/clam-mantle-spec` as the `MCP_HINT_KEYWORD` constant; the
...
> See the `clam-mantle-spec` README for rationale — why OpenAPI is
...
```bash
$ clam-mantle emit-openapi > ./openapi.json
```
...
- **Runtime engine** = zod (v3). The `@aotterclam/clam-mantle-spec`
...
> See the `clam-mantle-cloudflare` README for rationale — why D1 for
...
(`@aotterclam/clam-mantle-spec`); different collections coexist on the
...
(`@aotterclam/clam-mantle-netlify`) is a README stub for v0.2; its
