...
1. **`x-clam-bind`** (Schema property extension) — declares that
...
```yaml
authorId: { x-clam-bind: "${session.user.id}" }
createdAt: { x-clam-bind: "${request.timestamp}" }
```
...
### `x-clam-bind` — closed enum (v0.1)

```ts
type ClamBindValue = "ctx.user" | "ctx.staff" | "now";
```
...
New `x-clam-bind` values or `ctx.*` predicates do not get added
...
- AI author hit rate goes way up: a closed enum of 3 values
  (`x-clam-bind`) is unambiguous; the model outputs the right
...
**(d) `x-clam-bind: ctx.user.id`, `ctx.user.email`,
