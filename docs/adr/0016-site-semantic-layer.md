...
mantle consumer projects are agent-authored (ADR-0007). Once the
...
| `mantle/site.md` | Mantle (the mantle install / customize / deploy persona — Epic #47) | ~300 lines | Frontmatter + section bodies |
...
`create-mantle` fills at install time. Mantle then replaces the HTML
...
`create-mantle` substitutes these across templates:
...
versions before `create-mantle` learns to substitute them.
...
agent harness looks. `mantle/` is a mantle-owned subdirectory; the
...
- `mantle-starters/<archetype>/overlay/AGENTS.md.append` — content
  appended after the base.
- `mantle-starters/<archetype>/overlay/mantle/site.md.append` —
  section-keyed. v0.1: only allow appending to `## history` and adding
  new `## <name>` sections.

Overlay merging is the responsibility of `create-mantle`, not
Mantle.
...
- Placeholder substitution adds a step to install. Mitigated by
  collapsing into `create-mantle` (ADR-0013 step 3 → 4).
...
- **Single canonical file.** Smaller, but conflates "this is how to
  ...
  means" (mantle concern). The two layers churn at different rates;
...
3. Add to `create-mantle` substitution pass.
...
- Templates land in `mantle-starters/_common/` (sibling issue #101).
- Substitution is implemented in `packages/create-mantle` (#102).
- Install SKILL emits the post-`create-mantle` prose fill workflow
  (#103).
