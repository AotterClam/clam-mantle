...
**Five ports**, defined as TypeScript interfaces in `@aotter/mantle-runtime/src/ports/`. Concrete adapters provide implementations and inject them at module init via a single factory call.
...
- A future Bun/Deno/Vercel/Netlify port author reads this ADR + reads the 5 port interface files in `mantle-runtime/src/ports/` + writes 5 port impl files. That's the contract. No hidden state, no implicit assumptions about the HTTP framework.
