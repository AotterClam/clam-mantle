...
```ts
// packages/clam-cms-runtime/src/ports/database.ts (intent — exact shape lands in commit 4)
...
**(d) Plugin pattern (each port is a separate package)** — `@aotterclam/clam-cms-port-database`, `@aotterclam/clam-cms-port-kv`, etc., and runtime depends on all five. **Rejected**: 5 ports are too few to warrant 5 packages. The current 5-package structure (spec / runtime / admin-ui / cloudflare / netlify) is already at the boundary of "too many"; splitting further increases the maintenance tax without useful benefit. Ports are TS interfaces in `clam-cms-runtime`'s `src/ports/` directory — that's enough.
...
1. Read `clam-cms-runtime/src/ports/`. Implement each port against your runtime's primitives.
...
- [ ] Port interface files: `clam-cms-runtime/src/ports/{database,kv,session,assets,oauth}.ts` (commit 4)
- [ ] Port impls: `clam-cms-cloudflare/src/ports/*.ts` (commit 6)
