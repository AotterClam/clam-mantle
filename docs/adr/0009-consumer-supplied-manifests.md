...
The consumer's config file remains `src/clam.config.ts` (unchanged
...
`yaml.d.ts`, and the `src/clam.config.ts` file that wires manifests
...
1. Copy the relevant `starters/<name>/` contents into the consumer's
   ...
   `wrangler.toml`, and `src/clam.config.ts` all come along.
2. Wire `import postsYaml from "../manifests/posts.yaml"` (and
   peers) in `src/clam.config.ts`. No `?raw` suffix needed —
...
- The starter at `starters/blog/` is self-contained: `manifests/`,
  ...
  `src/clam.config.ts` wiring it together.
