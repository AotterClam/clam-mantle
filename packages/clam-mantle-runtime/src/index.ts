/**
 * `@aotterclam/clam-mantle-runtime` — public surface.
 *
 * Adapter-agnostic runtime engine for clam-mantle. Layered per the
 * Aotter clean-architecture convention (mirrors `aotter-clam/clam/core`):
 *
 *   domain ← usecase ← infrastructure ← runtime.ts (assembly root)
 *
 * Adapters (e.g. `@aotterclam/clam-mantle-cloudflare`) implement the
