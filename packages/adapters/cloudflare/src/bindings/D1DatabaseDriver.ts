...
class D1Migrations implements MigrationRunner {
  ...
  async runAll(migrations: ReadonlyArray<Migration>): Promise<void> {
    ...
    // `_clam_migrations`. Copy any not-yet-canonical rows over (no-op
    // on collisions) and drop the legacy table. Idempotent after the
    // first successful boot.
    const legacy = await this.db
      .prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='_clam_migrations'`)
      .first<{ name: string }>();
    if (legacy) {
      ...
      await this.db.batch([
        this.db.prepare(
          `INSERT OR IGNORE INTO _migrations (id, applied_at) SELECT id, applied_at FROM _clam_migrations`,
        ),
        this.db.prepare(`DROP TABLE IF EXISTS _clam_migrations`),
      ]);
    }
