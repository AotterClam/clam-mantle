...
function wrapCard(locale: ConsentLocale, inner: string): string {
  ...
  return `<!doctype html><html lang="${htmlLang}" dir="${direction}"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>${strings.title}</title><style>${CSS}</style></head><body><main class="card">${AOTTER_CLAM_MARK}${inner}</main></body></html>`;
}
