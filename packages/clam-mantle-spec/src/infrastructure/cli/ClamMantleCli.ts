#!/usr/bin/env node
/**
 * `clam-mantle` CLI dispatcher. Subcommands route to their implementing
...
async function main(): Promise<number> {
  ...
  if (!sub || sub === "--help" || sub === "-h") {
    stdout.write(`clam-mantle - SDK authoring CLI

Usage: clam-mantle <subcommand> [options]
...
Run \`clam-mantle <subcommand> --help\` for subcommand details.
`);
    return sub ? 0 : 2;
  }
