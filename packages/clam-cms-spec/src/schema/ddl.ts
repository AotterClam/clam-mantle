 ...
 * strings — no env / driver coupling. The runtime adapter (clam-cms-
 * runtime + a concrete D1 / Pg / SQLite binding) feeds these to the
 * SQL engine.
 *
 * Stays in `clam-cms-spec` because it's manifest-shape → SQL-string
 * with zero runtime concerns; a build-time validator + a CLI
 * (`clam-cms validate` / future `clam-cms openapi`) want to inspect
 * the planned DDL without booting an adapter, and the runtime's
 * migration emitter wants the same string-builder. One source, two
 * consumers.
 */
