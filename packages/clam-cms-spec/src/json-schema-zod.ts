 ...
 * Workers-safe.
 *
 * Coverage = exactly the keywords v0.1 grammar uses (see
 * `docs/design-atoms.md` and the starter manifests). DRAFT future
 ...
 * (`~` → `~0`, `/` → `~1`). Use this anywhere a zod issue's `path`
 * needs to be exposed in the same wire shape Ajv used to emit, so
 * downstream diagnostic consumers don't have to know which engine
 * produced the error.
 */
