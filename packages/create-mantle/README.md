...
1. Resolves `<archetype>` against the bundled source map → starters
   monorepo (`aotter/mantle-starters` for public archetypes;
   `aotter/mantle-starters-premium` for private — deferred).
...
3. Make sure `mantle-starters` already has the directory the entry
